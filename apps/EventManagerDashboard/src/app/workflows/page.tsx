    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage your workflow automations</p>
        </div>
              ...
              <tbody>
                {workflows.map((workflow) => (
                  <tr key={workflow.id} className="border-b">
                    ...
                    <td className="px-4 py-2">{workflow.status}</td>
                    <td className="px-4 py-2">{workflow.description || '-'}</td>
                    <td className="px-4 py-2 space-x-2">
                      <Button variant="outline" size="sm" onClick={() => openEdit(workflow)}>
                        <Edit className="h-4 w-4" />
                      </Button>
                      <Button variant="destructive" size="sm" onClick={() => setDeleting(workflow)}>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4">{editing ? 'Edit Workflow' : 'Create Workflow'}</h2>
